...
public class EmployeeShiftService {
    ...
    public void saveShiftData(String month, int year, MultipartFile file) {
        try (Workbook workbook = new XSSFWorkbook(file.getInputStream())) {
            ...
            while (rows.hasNext()) {
                ...
                Employee employee = employeeRepository.findByEmployeeNameIgnoreCase(employeeName).orElse(null);
                if (employee == null) {
    ...
    public EmployeeShift getEmployeeDetails(String month, int year, String employeeName) {
        List<EmployeeShift> employeeShifts = employeeShiftRepository.findByMonthAndYearAndEmployee_EmployeeName(month, year, employeeName);
    ...
    @Transactional
    public List<EmployeeShift> updateEmployeeDetails(String employeeName, EmployeeShift updatedEmployeeShift) {
        

        List<EmployeeShift> employeeShifts = employeeShiftRepository.findByEmployee_EmployeeNameIgnoreCase(employeeName);
        ...
        for (EmployeeShift employeeShift : employeeShifts) {
            if (updatedEmployeeShift.getEmployee() != null) {
                ...
                // Fetch employee again to verify the update
                Employee updatedEmployee = employeeRepository.findById(employee.getId()).orElseThrow(() -> new EmployeeNotFoundException("Updated employee not found"));
            }
        }
    ...
    @Transactional
    public void deleteEmployeeData(String employeeName) {
        List<EmployeeShift> employeeShifts = employeeShiftRepository.findByEmployee_EmployeeNameIgnoreCase(employeeName);
